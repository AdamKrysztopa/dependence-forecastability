"""Canonical showcase for Lag-Aware ModMRMR feature selection (LAM-F09, fast-scorer mode).

Runs the synthetic multi-covariate benchmark panel through
``run_lag_aware_mod_mrmr`` with a fast scorer configuration (mutual_info_sklearn +
spearman_abs) and emits JSON, tables, figures, and markdown summaries.

The ablation section demonstrates three live selection modes:

- **relevance-only**: post-hoc ranking of candidates by normalised relevance
- **ModMRMR (no target-history)**: full Lag-Aware ModMRMR without the
  target-history novelty penalty
- **full Lag-Aware ModMRMR**: ModMRMR with target-history novelty scoring at lag 12

The aggregate-mean and mean-similarity multiplicative baselines are discussed as
conceptual prose in the summary markdown.  They require internal per-pair redundancy
data not exposed by the public result surface.

Usage::

    uv run scripts/run_showcase_lag_aware_mod_mrmr.py
    uv run scripts/run_showcase_lag_aware_mod_mrmr.py --smoke
...
_SHOWCASE_NAME: str = "showcase_lag_aware_mod_mrmr"
_RELEASE: str = "0.4.3"


def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Canonical Lag-Aware ModMRMR showcase (LAM-F09, fast-scorer mode)"
    )
    ...
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="CI-friendly preset: n=500, candidate_lags=[1..6]",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
...
def _resolved_settings(args: argparse.Namespace) -> dict[str, object]:
    if args.smoke:
        n: int = 500
        candidate_lags: list[int] | None = [1, 2, 3, 4, 5, 6]
        max_selected: int = 5
...
def _build_config(*, settings: dict[str, object]) -> LagAwareModMRMRConfig:
    ...
    redundancy_spec = PairwiseScorerSpec(
        name="spearman_abs",
        backend="scipy",
        normalization="rank_percentile",
        significance_method="none",
    ...
    return LagAwareModMRMRConfig(
        ...
        redundancy_scorer=redundancy_spec,
        target_history_scorer=redundancy_spec,
    )
...
def _build_stub_triage_result(result: LagAwareModMRMRResult) -> TriageResult:
    ...
    return TriageResult(
        ...
        interpretation=InterpretationResult(
            ...
            narrative="lag-aware showcase narrative",
...
def _build_stub_routing() -> RoutingRecommendation:
    return RoutingRecommendation(
        ...
        rationale=["lag-aware showcase routing stub"],
        caution_flags=[],
...
def _verify_result(result: LagAwareModMRMRResult, *, smoke: bool = False) -> list[str]:
    ...
    # In smoke mode (small n, fast scorer) the near-duplicate sensor_near_dup may absorb
    # this slot due to higher raw relevance.  Accept either member of the duplicate pair
    # at lag 3 as satisfying the causal-signal recovery criterion.
        ...
        if dup_rank < driver_rank:
            if smoke:
                _logger.warning(
                    ...
                    "in smoke mode (small-n artifact; acceptable).",
                    _dup_other,
                    dup_rank,
                    _TRUE_DRIVER_NAME,
                    driver_rank,
...
def _save_relevance_profile(path: Path, result: LagAwareModMRMRResult) -> None:
    ...
    ax.set_xlabel("Normalised relevance score")
    ax.set_title("Relevance Profile — Lag-Aware ModMRMR (full with target-history)")
...
def _save_ablation_heatmap(
    ...
    *,
    ...
    ax.set_title("Ablation Comparison — Feature Selection by Mode")
...
def _build_summary_markdown(
    *,
    ...
    lines: list[str] = [
        "# Lag-Aware ModMRMR Showcase Summary (Fast Scorer)",
        "",
        "<!-- type: reference -->",
        "",
        "## Settings",
    ...
    lines.extend(
        [
            ...
            "## Ablation Comparison",
            "",
            "Three live ablation modes are shown. Two further baselines "
            "(aggregate-mean and mean-similarity multiplicative) are discussed "
            "below as conceptual references.",
            "",
    ...
    lines.extend(
        [
            ...
            "",
            "Candidates ranked by normalised relevance score (no redundancy penalty):",
            "",
    ...
    lines.extend(
        [
            ...
            "",
            "The following two baselines cannot be computed from the public result surface,",
            "as they require per-pair redundancy data internal to the selector.",
            "",
            "### Aggregate-Mean Redundancy Baseline",
            "",
            "The aggregate-mean form penalises each candidate by the **average** redundancy",
            "to all currently selected features:",
            "",
            "    score_mean(i) = relevance(i) * (1 - mean_{s in S} rho(f_i, f_s))",
            "",
            "In contrast, the implemented Lag-Aware ModMRMR uses the **maximum**:",
            "",
            "    score_max(i) = relevance(i) * (1 - max_{s in S} rho(f_i, f_s))",
            "",
            "The fixture "
            "`docs/fixtures/lag_aware_mod_mrmr/expected/aggregate_redundancy_vs_maximum.json`"
            " demonstrates a synthetic case where these two strategies "
            "produce different selection sets.",
            "",
            "### Mean-Similarity Multiplicative Baseline",
            "",
            "A mean-similarity baseline applies the mean (rather than max) redundancy in the",
            "multiplicative suppression formula.  This is a weaker suppression rule: a candidate",
            "with moderate similarity to several already-selected features is penalised less than",
            "under max-similarity.  Refer to the aggregate_redundancy_vs_maximum fixture for the",
            "contrast case.",
            "",
            "## Verification",
            "",
...
def _build_verification_markdown(*, violations: list[str]) -> str:
    ...
    lines = [
        "# Lag-Aware ModMRMR Showcase Verification",
        "",
        f"- status: **{status}**",
        "",
        "## Violations",
...
def main(argv: list[str] | None = None) -> int:
    """Run the Lag-Aware ModMRMR showcase and return POSIX-style exit code."""
    ...
    _say(quiet, "=" * 72)
    _say(quiet, "[lam-showcase] LAM-F09 lag-aware ModMRMR showcase (fast scorer)")
    _say(quiet, "=" * 72)
    _say(quiet, f"[lam-showcase] mode            : {'smoke' if args.smoke else 'full'}")
    _say(quiet, f"[lam-showcase] n               : {settings['n']}")
    _say(quiet, f"[lam-showcase] random_state    : {args.random_state}")
    _say(quiet, f"[lam-showcase] output_root     : {output_root}")
    _say(quiet, "")
    ...
    _say(
        quiet,
        f"[lam-showcase] step 1/5  generated panel [{time.perf_counter() - t0:.2f}s]",
    )
    ...
    result: LagAwareModMRMRResult = run_lag_aware_mod_mrmr(
        ...
        run_id="full_mod_mrmr",
    )
    _say(
        quiet,
        f"[lam-showcase] step 2a/5 ran full ModMRMR (with TH) [{time.perf_counter() - t1:.2f}s]",
    )
    ...
    result_no_th: LagAwareModMRMRResult = run_lag_aware_mod_mrmr(
        ...
        config=config_no_th,
        random_state=int(args.random_state),
        run_id="mod_mrmr_no_th",
    )
    _say(
        quiet,
        f"[lam-showcase] step 2b/5 ran ModMRMR (no TH) [{time.perf_counter() - t2:.2f}s]",
    )
    ...
    _say(
        quiet,
        f"[lam-showcase] step 3/5  built contract [{time.perf_counter() - t3:.2f}s]",
    )
    ...
    _say(
        quiet,
        f"[lam-showcase] step 4/5  rendered figures [{time.perf_counter() - t4:.2f}s]",
    )
    ...
    _say(
        quiet,
        f"[lam-showcase] step 5/5  wrote artifacts [{time.perf_counter() - t5:.2f}s]",
    )
    for artifact_path in [
        ...
        _say(quiet, f"[lam-showcase]           -> {artifact_path}")
