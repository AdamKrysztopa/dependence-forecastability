...
class ForecastabilityAnalyzer:
    ...
    def plot(self, *, method: str | None = None, show: bool = True) -> Any:
        ...
        # Resolved through importlib (not a static import) so use_cases keeps no
        # static import edge into the adapters layer (hexagonal boundary).
        import importlib

        plot_analyzer = importlib.import_module(
            "forecastability.adapters.analyzer_plot"
        ).plot_analyzer
