"""Canonical live-LLM triage adapter (AGT-008a).

This is the canonical location for the PydanticAI adapter that wraps the
deterministic triage pipeline and exposes it to an LLM orchestrator.
It lives under the ``adapters/agents/runtime/`` path.

The adapter exposes ``run_triage_agent()`` and its sub-steps as PydanticAI
tools, letting an LLM orchestrate and *explain* the deterministic results —
without ever generating numeric values itself.
...
from forecastability.domain.results.agent_tools.triage_agent_results import (
...
def _readiness_to_result(report: ReadinessReport) -> ValidateSeriesResult:
    """Serialise a ReadinessReport to a frozen result model for the LLM."""
    return ValidateSeriesResult(
        status=report.status.value,
        warnings=[ReadinessWarningResult(code=w.code, message=w.message) for w in report.warnings],
    )


def _method_plan_to_result(plan: MethodPlan) -> MethodPlanResult:
    """Serialise a MethodPlan to a frozen result model for the LLM."""
    return MethodPlanResult(
        route=plan.route,
...
def _diagnostics_to_str(diagnostics: dict[str, float | int] | None) -> str | None:
    """Serialise analytics diagnostics dict to a compact string for the LLM.

    Args:
        diagnostics: Optional dict of diagnostic numeric values.

    Returns:
        Compact string representation or ``None`` when absent.
    """
    if diagnostics is None:
        return None
    return ", ".join(f"{k}={v}" for k, v in diagnostics.items())


def _triage_result_to_run_result(result: TriageResult) -> TriageRunResult:
    """Serialise a TriageResult to a frozen :class:`TriageRunResult` for the LLM.

    Avoids passing large numpy arrays; instead provides summary statistics.
    """
    view = present_triage_result(result)
    # readiness_warnings are list[dict[str, str]] with "code" and "message" keys
    readiness = ValidateSeriesResult(
        ...
        warnings=[
            ...
            for w in view.readiness_warnings
        ],
    )

    method_plan: MethodPlanResult | None = None
    if view.route is not None:
        method_plan = MethodPlanResult(
            route=view.route,
            compute_surrogates=view.compute_surrogates or False,
            assumptions=view.method_plan_assumptions or [],
            rationale=view.method_plan_rationale or "",
        )
    ...
    if view.method is not None and analytics is not None:
        analyze_summary = AnalyzeSummaryResult(
            method=view.method,
            recommendation=view.recommendation or "",
    ...
    if view.forecastability_class is not None:
        interpretation = InterpretationResult(
            ...
            directness_class=view.directness_class or "",
            primary_lags=list(view.primary_lags),
            modeling_regime=view.modeling_regime or "",
            narrative=analytics.narrative if analytics is not None else None,
            diagnostics=(
                _diagnostics_to_str(analytics.diagnostics) if analytics is not None else None
            ),
        )

    timing_ms: dict[str, float] | None = None
    if result.timing:
        timing_ms = {k: float(v) for k, v in result.timing.items()}
...
def create_triage_agent(
    *,
    ...
    @agent.tool
    def validate_series(ctx: RunContext[TriageDeps]) -> ValidateSeriesResult:
        ...
        return _readiness_to_result(report)
    ...
    @agent.tool
    def plan_analysis(ctx: RunContext[TriageDeps]) -> MethodPlanResult | ReadinessBlockedResult:
        ...
        if readiness.status == ReadinessStatus.blocked:
            return ReadinessBlockedResult(
                error="Series is blocked by readiness gate",
                readiness=_readiness_to_result(readiness),
            )
        plan = plan_method(request, readiness)
        return _method_plan_to_result(plan)
    ...
    @agent.tool
    def run_full_triage(ctx: RunContext[TriageDeps]) -> TriageRunResult:
        ...
        return _triage_result_to_run_result(result)
    ...
    @agent.tool
    def list_available_scorers(ctx: RunContext[TriageDeps]) -> list[ScorerInfoResult]:
