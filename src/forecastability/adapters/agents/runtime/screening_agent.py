...
from forecastability.domain.results.agent_tools.screening_agent_results import (
...
def _result_summary(result: TriageResult) -> TargetAssessmentResult:
    """Condense a ``TriageResult`` into a typed result for LLM consumption."""
    ...
    if result.analyze_result is not None:
        peak_raw = round(float(result.analyze_result.raw.max()), 4)
        if result.analyze_result.partial is not None:
            peak_partial = round(float(result.analyze_result.partial.max()), 4)

    if result.interpretation is not None:
        forecastability = result.interpretation.forecastability_class
        directness = result.interpretation.directness_class
        regime = result.interpretation.modeling_regime
        narrative = result.interpretation.narrative

    return TargetAssessmentResult(
        blocked=result.blocked,
        peak_raw=peak_raw,
        peak_partial=peak_partial,
...
def _feature_screen_result(
    feature_name: str,
    result: TriageResult,
) -> FeatureScreenResult:
    """Build a typed :class:`FeatureScreenResult` from a triage result."""
    base = _result_summary(result)
    return FeatureScreenResult(
        feature=feature_name,
        blocked=base.blocked,
        peak_raw=base.peak_raw,
        peak_partial=base.peak_partial,
        forecastability=base.forecastability,
...
def create_screening_agent(
    ...
    *,
    ...
    @agent.tool
    def list_candidates(ctx: RunContext[ScreeningDeps]) -> dict[str, CandidateStatsResult]:
        """List candidate features with lightweight descriptive statistics."""
        return {
            name: CandidateStatsResult(
                n=len(arr),
    ...
    @agent.tool
    def assess_target(ctx: RunContext[ScreeningDeps]) -> TargetAssessmentResult:
    ...
    def screen_feature(
        ...
        feature_name: str,
    ) -> FeatureScreenResult | UnknownFeatureResult:
        ...
        return _feature_screen_result(feature_name, result)
