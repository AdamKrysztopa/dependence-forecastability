...
:mod:`forecastability.adapters.agents.runtime.triage_agent`.  It wraps the
deterministic :class:`FingerprintBundle` behind a PydanticAI agent so that a
language model can *narrate* the fingerprint and routing findings without
introducing numeric values, inventing model recommendations, or overriding
...
from forecastability.domain.results.agent_tools.fingerprint_agent_results import (
...
def create_fingerprint_agent(
    ...
    * ``get_interpretation`` — retrieves the deterministic A3 interpretation from
    ...
    @agent.tool
    def run_fingerprint(ctx: RunContext[FingerprintDeps]) -> RunFingerprintResult:
        """Compute the forecastability fingerprint for the target series.

        Returns a typed result with geometry, fingerprint, and routing fields,
    ...
    def get_interpretation(
        ctx: RunContext[FingerprintDeps],
    ) -> GetInterpretationResult | GetInterpretationErrorResult:
        """Retrieve the deterministic A3 interpretation for the cached fingerprint.

        Returns a typed result with structure_bucket, confidence_label,
        deterministic_summary, rich_signal_narrative, cautionary_narrative,
        and evidence.
        Requires that ``run_fingerprint`` has been called first.
        """
        deps = ctx.deps
        if deps._bundle is None:
            return GetInterpretationErrorResult(
                error="Fingerprint not yet computed. Call run_fingerprint first."
            )
        ...
        evidence = InterpretationEvidenceResult(
            information_structure=interpretation.evidence.information_structure,
            confidence_label=interpretation.evidence.confidence_label,
            information_horizon=interpretation.evidence.information_horizon,
            informative_horizon_count=interpretation.evidence.informative_horizon_count,
            information_mass_bucket=interpretation.evidence.information_mass_bucket,
            nonlinear_share_bucket=interpretation.evidence.nonlinear_share_bucket,
            signal_to_noise_bucket=interpretation.evidence.signal_to_noise_bucket,
            has_directness_ratio=interpretation.evidence.has_directness_ratio,
            caution_count=interpretation.evidence.caution_count,
        )
        return GetInterpretationResult(
            ...
            caution_flags=list(interpretation.caution_flags),
            rationale=list(interpretation.rationale),
            evidence=evidence,
        )
