...
| EX-MIG-03 | Pin framework deps in sibling repo | 2 | 2a | Sibling repo `pyproject.toml` declares `darts`, `mlforecast`, optionally `statsforecast`, `nixtla` under dev/extras | Proposed |
| **EX-NB-LOCK-01** | **Lockfile-pinned reproducibility** | 2 | 2a | Commit `uv.lock` in sibling repo; CI uses `uv sync --frozen`; quarterly Dependabot / scheduled refresh PR bumps the lockfile and merges only if the matrix is green. | Proposed |
| **EX-NB-MATRIX-01** | **Two-axis sibling CI matrix** | 2 | 2a | `python ∈ {3.11, 3.12}` × `source ∈ {pinned, unpinned-main}`. `pinned` is a required check (gates merge); `unpinned-main` runs with `continue-on-error: true` to surface upstream framework drift early without blocking PRs. | Proposed |
| EX-MIG-02 | Sibling repo nbconvert CI | 2 | 2a | Add a workflow that executes notebooks against PyPI-installed `dependence-forecastability >= 0.4.0` | Proposed |
...
| **EX-NB-02** | **Sibling recipe `recipes/contract_roundtrip.ipynb`** | 2 | 2b | `ForecastPrepContract.model_dump_json()` → disk → `model_validate_json()` → re-validate against a fresh triage run. Final cell consumes the JSON without importing `forecastability` to demonstrate the framework-agnostic hand-off boundary. | Proposed |
...
| **EX-NB-DATA-01** | **Per-notebook data-shipping policy** | 2 | 2c | Sibling `data/README.md` reproduces the per-notebook origin table (§5.4): small public CSVs (Air Passengers, ETTh1 OHT subset) ship in the sibling repo; M4 and causal-rivers fetch via a port of `scripts/download_data.py` cached under `actions/cache@v4`. | Proposed |
...
| EX-CR-01 | Remove `notebooks/` | 3 | 3a | Delete the directory in the core repo on the v0.4.0 branch | Proposed |
| EX-CR-02 | Remove `outputs/notebook_runs/` | 3 | 3a | Delete the directory and verify no script writes there | Proposed |
| EX-CR-03 | Remove notebook references from docs | 3 | 3a | Drop notebook callouts from `README.md`, `docs/quickstart.md`, `docs/notebooks/`, `llms.txt`, `AGENTS.md`, `.github/copilot-instructions.md` | Proposed |
| EX-CR-04 | Remove `scripts/check_notebook_contract.py` | 3 | 3a | Delete the script and any tests referencing it | Proposed |
