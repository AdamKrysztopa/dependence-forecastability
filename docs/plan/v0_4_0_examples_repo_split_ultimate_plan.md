...
**Batch 4a landed:** 2026-04-29 — EX-D03 (`docs/examples_index.md` created with 12 walkthroughs, 6 triage walkthroughs, 1 recipe, issues table, cross-ref; sibling `walkthroughs/`, `triage_walkthroughs/`, `recipes/` READMEs updated with full notebook tables), EX-D04 (`llms.txt` updated to 7-item Start Here list with recipes entry at position 4), EX-D06 (`AGENTS.md` Navigation Order and `.github/copilot-instructions.md` Start Here both updated to 7 items with recipes entry)
...
| EX-D03 | Forwarding section in README and `docs/examples_index.md` | 4 | 4a | New section in README; new index page listing every migrated notebook with destination URL | Implemented |
| EX-D04 | Update llms.txt and start-here anchors | 4 | 4a | Drop notebook anchor; add recipes/examples-index anchor | Implemented |
| EX-D06 | Update `.github/copilot-instructions.md` and `AGENTS.md` "Start Here" lists | 4 | 4a | Replace `notebooks/walkthroughs/...` anchor with `docs/recipes/` and `docs/examples_index.md` | Implemented |
| EX-D05 | Refresh `docs/implementation_status.md` and `docs/surface_guide.md` | 4 | 4b | Reflect the new repo split | Implemented |
| **EX-NB-04** | **Bidirectional recipe cross-link** | 4 | 4b | Sibling `recipes/README.md` links forward to executed-notebook release assets and back to the core text recipe at [docs/recipes/forecast_prep_to_external_frameworks.md](../recipes/forecast_prep_to_external_frameworks.md). Core recipe page gains a forward link to EX-NB-01 / EX-NB-02. Mitigates doc-rot risk between the two repos. | Implemented |
| EX-CHG-01 | CHANGELOG entry for `0.4.0` | 5 | 5a | "Library-first slim release" entry with the migration note, rationale link, and a forward link to the sibling repo's v0.3.4 sprint-showcase notebook (EX-NB-01) | Implemented |
| EX-R01 | Version bump and tag | 5 | 5a | `0.4.0` across `pyproject.toml`, `__init__.py`, `CITATION.cff`, `CHANGELOG.md`, `repo_contract.yaml`; tag `v0.4.0` (pending) | Implemented |
| **EX-REL-01** | **Two-repo release dance documented in sibling `RELEASING.md`** | 5 | 5b | Step-by-step: core RC1 → sibling pre-flight against TestPyPI / `git+` source → acceptance gate (sibling matrix green) → core final tag and PyPI publish → sibling pin bump and final tag → cross-repo handshake comment (EX-CPL-01). | Implemented |
| **EX-REL-02** | **TestPyPI / `git+` pre-flight matrix** | 5 | 5b | Sibling pre-flight matrix has a `source` axis with values `testpypi` and `git`. Either green satisfies the EX-REL-01 acceptance gate; both green is preferred. Exercises the actual `pip install` machinery before the irrevocable PyPI publish. | Implemented |
| EX-R02 | PyPI publish | 5 | 5b | Trusted Publishing flow as in `docs/plan/pypi_release_plan.md`; sequenced inside the two-repo release dance (EX-REL-01) | Implemented |
| EX-R03 | Sibling repo first release | 5 | 5b | Cut sibling first release pinning `dependence-forecastability == 0.4.0`; sequenced inside EX-REL-01 | Implemented |
| **EX-CPL-01** | **Cross-repo CI handshake** | 5 | 5b | Sibling repo's `release` workflow listens on `repository_dispatch` (`event_type = core_release`) emitted by core's release workflow. On firing, sibling re-runs the full notebook matrix against the new PyPI artifact and posts a check status / comment back to the core release page. Manual `workflow_dispatch` fallback for outages. | Implemented |
