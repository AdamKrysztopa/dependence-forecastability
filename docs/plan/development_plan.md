...
| A2 | Agent adapter | F1–F8 | 3 | A1 | `triage_summary_serializer.py` — result models → agent-safe payloads; schema version | ✅ Done |
| A3 | Agent adapter | F1–F8 | 3 | A1, A2 | `triage_agent_interpretation_adapter.py` — concise summaries; warnings; experimental flags | ✅ Done |
| N6 | Notebook | A1–A3 | 3 | — | `10_agent_ready_triage_interpretation` — triage → payload → summary → deterministic vs narrative | ✅ Done (updated for A3 adapter mapping + explicit warning/experimental handling) |
