...
| **[v0_5_0_review_hardening_ultimate_plan.md](implemented/v0_5_0_review_hardening_ultimate_plan.md)** | **v0.5.0 — review-driven hardening: math honesty, architecture realignment, efficiency reset** | **Shipped (v0.5.0, current released version)** |
| **[v0_5_1_finish_hex_migration_plan.md](implemented/v0_5_1_finish_hex_migration_plan.md)** | **v0.5.1 (patch) — finish the hexagonal migration: Strangler-Fig cycle removal (HEX-*), first-party boundary test + cycle detection, `utils/` leaf split. Structure-only; additive deprecations, public surface byte-stable.** | **Shipped (v0.5.1; landed on `main`; layer graph acyclic, boundary test enforced)** |
...
- The [finish-hex-migration plan](implemented/v0_5_1_finish_hex_migration_plan.md) shipped as **v0.5.1** (landed on `main` via branch `refactor/finish-hex-migration`). It was structure-only (Strangler-Fig cycle removal) and shipped as a patch release with additive `DeprecationWarning` shims only; the top-level layer graph is now acyclic, `domain`/`ports` are first-party-closed, and `tests/test_architecture_boundaries.py` enforces no first-party leaks and no layer cycles behind a byte-stable public surface.
