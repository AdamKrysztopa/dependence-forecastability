...
| **[v0_5_x_finish_hex_migration_plan.md](implemented/v0_5_x_finish_hex_migration_plan.md)** | **Post-v0.5.0 — finish the hexagonal migration: Strangler-Fig cycle removal (HEX-*), first-party boundary test + cycle detection, `utils/` leaf split. Structure-only, no release tag.** | **Complete (landed on `main`; layer graph acyclic, boundary test enforced)** |
...
- The post-v0.5.0 [finish-hex-migration plan](implemented/v0_5_x_finish_hex_migration_plan.md) is **complete** (landed on `main` via branch `refactor/finish-hex-migration`). It was structure-only (Strangler-Fig cycle removal) and shipped no release tag; the top-level layer graph is now acyclic, `domain`/`ports` are first-party-closed, and `tests/test_architecture_boundaries.py` enforces no first-party leaks and no layer cycles behind a byte-stable public surface.
