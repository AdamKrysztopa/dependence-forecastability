...
routing label. Reviewed against the source literature and updated to scope
every statistical claim to the API surface it actually applies to.*
...
One precision note worth stating carefully: KSG-II is the implementation
choice used to estimate Catt's horizon-specific forecastability profile.
Catt's theoretical result is about the information quantity under a declared
information set — it is independent of which specific numerical kernel is
used to estimate it. The switch to KSG-II is paper-consistent, in the sense
that Chebyshev-metric, median-over-`k` estimation is specified in the local
AMI Information Geometry working note; it is not a claim that every
published source mandates this exact kernel.
...
First, the default number of phase-randomised surrogates rose from **99 to
999 at the top-level triage request** (`TriageRequest.n_surrogates`). That
raises p-value resolution from 0.01 to 0.001 for users of the high-level
`run_triage` entry point. Some lower-level and legacy helpers retain smaller
defaults for compatibility and runtime: `ForecastabilityAnalyzer` still
defaults to 99, the raw `compute_significance_bands` helper defaults to 99,
and `AmiInformationGeometryConfig` uses 200 (matching the local AMI geometry
working note). If you use those surfaces directly, the improved resolution
does not apply automatically — you must pass `n_surrogates=999` explicitly.
The validation floor (the minimum accepted value across all surfaces) is
still 99. The batched FFT path shipped in this release means 10× more
surrogates at the triage level cost roughly 3–4× wall-clock, not 10×.

For reference, the surrogate and correction defaults by API surface are:

| Surface | `n_surrogates` default | Correction default |
|---|---:|---|
| `TriageRequest` / `run_triage` | **999** | Romano-Wolf (via correction service) |
| `ForecastabilityAnalyzer` | 99 | legacy percentile bands |
| `compute_significance_bands` | 99 | `'none'` (percentile bands) |
| `compute_significance_bands_corrected` | 99 | Romano-Wolf |
| `AmiInformationGeometryConfig` | 200 | Romano-Wolf |

Papers and reports using this toolkit should state which surface and which
`n_surrogates` value was used, since the effective p-value resolution
differs by an order of magnitude between the 99 and 999 defaults.

A second change affects what the phase-randomised surrogate null hypothesis
actually is. Phase surrogates preserve the power spectrum and therefore
preserve linear autocorrelation structure while disrupting phase-specific
nonlinear dependencies. They test the null that the observed AMI value could
have arisen from a linear process with the same spectrum, not an independent
or identically distributed null. For strongly periodic series, phase
surrogates may retain significant AMI structure; the corrected band then
tests only whether the nonlinear component exceeds what a matched linear
process produces.

Third, **Romano-Wolf step-down family-wise correction** across lags is now
the default in the correction service and the AMI information-geometry path.
The previous per-lag uncorrected behaviour is preserved in the legacy
`compute_significance_bands` helper behind `correction='none'`. Romano-Wolf
is a resampling-based step-down procedure; the test
...
- `compute_transfer_entropy_ksg` — true Schreiber transfer entropy via the
  ...
  the source provides beyond that. The quantity is directional conditional
  dependence in Schreiber's sense: `I(X_{past}; Y_{future} | Y_{past})`.
...
An important interpretive boundary applies to both surfaces: directional
conditional dependence is not the same as proof of causation. A
significant `compute_transfer_entropy_ksg` value means the source's past
provides predictive information about the target's future beyond what the
target's own past already provides — under the declared information set and
the stationarity assumptions of the estimator. Common drivers, hidden
confounders, and indirect paths are not ruled out by this test alone.
Causal interpretation requires either domain assumptions or a structured
causal-screening workflow such as PCMCI that explicitly conditions on and
significance-gates indirect paths.
...
this directly. The correct interpretation is ordinal: a positive pAMI value
means there is residual dependence at that lag after linear intermediate-lag
structure has been removed, but it does not quantify the nonlinear portion
of conditional MI.

Two naming clarifications are worth stating explicitly for readers familiar
with the information-theory literature. First, Liang et al. (2013) use the
acronym PAMI for *permutation auto-mutual information* — a different method
based on ordinal patterns. The repo's `pAMI` means *partial* AMI via linear
residualization and these are unrelated estimators with different
assumptions. Second, the `directness_ratio` (pAMI / AMI) is bounded in
`[0, 1]` by construction; values above 1.0 indicate a numerical or
scale issue, not evidence of a stronger direct lag effect, and v0.5.0 now
emits a warning when fingerprint surfaces encounter out-of-range ratios.
...
**A note on GCMI and estimator units.** Some internal paths (specifically
the low-cardinality and heavily tied-value branches of `compute_ami`)
reroute to the Gaussian-copula MI estimator from Ince et al. (2017). GCMI
is rank-based, efficient, and robust to ties, and it returns MI-style
effect sizes in bits under a Gaussian-copula approximation. KSG-II returns
estimates in natural-log units (nats) unless explicitly converted.
If you compare AMI values across series where some triggered the GCMI
reroute and some did not, you are mixing units, and any threshold or
ranking based on those values will have scale-dependent behaviour. The
toolkit's routing thresholds were calibrated on KSG nats; cross-estimator
comparisons need an explicit `/ np.log(2)` unit conversion before
thresholding on bit-scale values, or an acknowledgement that the
Gaussian-copula approximation is a lower bound under non-Gaussian copula
dependence rather than a direct substitute for the KSG estimate.
...
One scientific boundary worth making explicit: Catt's forecastability profile
establishes horizon-specific information limits under a declared information
set and is exact as a log-loss bound in the theoretical case. The toolkit's
model-family routing recommendations in `ForecastPrepContract` are a
deterministic heuristic layer built on top of those estimates. They are
audited against synthetic archetypes (see §3), but they have not been
validated against held-out downstream forecast accuracy under sMAPE, MSE, or
any other loss. Treat routing recommendations as a structured starting point
that should be cross-checked against domain knowledge and external evaluation
— not as guaranteed optima.
...
5. **Audit was run at `n_surrogates=99`.** The calibration script
   (`scripts/run_routing_confidence_calibration.py`) called
   `TriageRequest(..., n_surrogates=99, random_state=42)`. The reported
   precision numbers (HIGH 0.993, MEDIUM 0.584, LOW 0.752) are therefore
   measured under the v0.4.3-equivalent surrogate count, not under the
   new 999-surrogate default. Whether the higher-resolution surrogate
   test improves or changes routing precision is a separate question that
   the v0.5.1 threshold-sweep will address.
